...
const run = (cmd) => {
  try {
    return execSync(cmd, { cwd: projectRoot, encoding: "utf-8", timeout: 8000, windowsHide: true }).trim();
  } catch {
    return "";
  }
};
...
const pendingChanges = hasOpenSpec
  ? readdirSync(join(projectRoot, "openspec", "changes"))
      .filter((entry) => entry !== "archive" && !entry.startsWith("."))
      .join("\n")
  : "";
