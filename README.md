...
On the annotation canvas, use the mouse wheel to zoom around the cursor and hold the left button to pan the image. Click-based marking still works on the canvas itself. Mouse mode can adjust existing boxes, polygon points, and pose keypoints without creating new annotations; visible points and resize handles take priority over bbox interiors when they overlap. The instance panel uses readable labels such as pose, box, and polygon summaries; clicking a row selects that instance on the canvas, including the pose bbox and visible keypoints. Annotation shortcuts: `V`/`M` mouse mode, `B` box mode, `P` polygon mode, `K` keypoint mode, `[`/`]` previous or next keypoint, `+`/`-` zoom, `0` reset zoom, arrow keys switch images, `Ctrl+S` saves, and `Ctrl+Z`/`Ctrl+Y` undo or redo. Saved station annotations preserve nested image paths under the project annotations directory, so same-basename images in different folders keep separate annotation files.
