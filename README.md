...
On the annotation canvas, use the mouse wheel to zoom around the cursor and hold the left button to pan the image. Click-based marking still works on the canvas itself. The left image list scrolls independently when a project has many images. Use the toolbar `标签` dropdown to choose the class for newly created boxes, polygons, or pose instances. Mouse mode can adjust existing boxes, polygon points, and pose keypoints without creating new annotations; visible points and resize handles take priority over bbox interiors when they overlap. The instance panel uses readable labels such as pose, box, and polygon summaries; clicking a row selects that instance on the canvas, including the pose bbox and visible keypoints. Use each instance row's `标签` dropdown to change an existing instance class, `隐藏` to temporarily remove that instance from the canvas, or `删除` to remove the whole instance. Use the class visibility chips above the instance list to hide or show all annotations for one label while continuing to mark other labels; when many instances exist, the instance list scrolls independently. Annotation shortcuts: `V`/`M` mouse mode, `B` box mode, `P` polygon mode, `K` keypoint mode, `[`/`]` previous or next keypoint, `+`/`-` zoom, `0` reset zoom, arrow keys switch images, `Ctrl+S` saves, and `Ctrl+Z`/`Ctrl+Y` undo or redo. Saved station annotations preserve nested image paths under the project annotations directory, so same-basename images in different folders keep separate annotation files.

Use the project center import form to create a project from a backend-accessible image directory, or choose local image files/folders in the browser and upload them into a new project. Annotation files can be chosen directly from the browser or entered as backend-accessible paths. After entering a project, use the `数据/导入` page to import or rematch annotations into that current project. You can now multi-select annotation files, including many `.txt` files at once, and the importer will match them against image names, basenames, or stems automatically. Supported annotation sources include YOLO TXT label folders, CVAT XML, COCO JSON, LabelMe JSON, and Pascal VOC XML; matching uses exact image names, basenames, or stems.

Use the `模型工作台` button in the top bar to work outside the project annotation/training tabs. The workbench has two clear interfaces: `预测` for single-image prediction, and `预标注` for writing model predictions into a target project. Both interfaces can reuse a project's trained `best.pt`, `last.pt`, or exported ONNX artifacts, or switch to a local `.pt` / `.onnx` path. Pre-labeling only writes annotations for images that do not already have labels, so existing human annotations are skipped.
